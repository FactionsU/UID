...
public class CmdStuck implements Cmd {
    ...
    public TriConsumer<CommandManager<Sender>, Command.Builder<Sender>, MinecraftHelp<Sender>> consumer() {
        var tl = Confs.tl().commands().stuck();
    ...
    private void handle(CommandContext<Sender> context) {
        ...
        final long delay = Math.max(1, Confs.main().commands().stuck().getDelay());
        final int radius = Confs.main().commands().stuck().getRadius();
        final int searchRadius = Confs.main().commands().stuck().getSearchRadius();
        ...
        if (waiting.contains(sender.uniqueId()) || (sender.warmup() instanceof WarmUpUtil.Warmup warmup && warmup == WarmUpUtil.Warmup.STUCK)) {
            ...
            if (!context.sender().payForCommand(Confs.main().economy().getCostStuck(), econTl.getStuckTo(), econTl.getStuckFor())) {
                return;
            }
            ...
            WarmUpUtil.process(sender, WarmUpUtil.Warmup.STUCK,
                    ...
                    () -> {
                        ...
                        new SpiralTask(new FLocation(player), searchRadius) {
                            final int buffer = Confs.main().worldBorder().getBuffer();
