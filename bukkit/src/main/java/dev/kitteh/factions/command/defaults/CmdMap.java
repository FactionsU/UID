...
public class CmdMap implements Cmd {
        ...
        return (manager, builder, _) -> {
            ...
            manager.command(
                    builder.literal(tl.getFirstAlias(), tl.getSecondaryAliases())
                            ...
                            .flag(manager.flagBuilder("set-height").withComponent(IntegerParser.integerParser(1, Confs.main().map().getHeight() * 2)))
                            .handler(this::handle)
            );
        };
    }
    ...
    private void handle(CommandContext<Sender> context) {
        ...
        if (!context.sender().payForCommand(Confs.main().economy().getCostMap(), econTl.getMapTo(), econTl.getMapFor())) {
            return;
        }
