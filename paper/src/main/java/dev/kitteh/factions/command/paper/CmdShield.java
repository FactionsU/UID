...
public class CmdShield extends dev.kitteh.factions.command.defaults.CmdShield {
    ...
    private void handleSchedule(CommandContext<Sender> context) {
        ...
        this.statusDialog(fPlayer, fPlayer.asPlayer());
    }
    ...
    private void statusDialog(FPlayer fPlayer, Player player) {
        ...
        buttons.add(ActionButton.builder(Mini.parse(tl.getScheduleMenuStatusButtonSetSchedule(), fPlayer, factionResolver))
                .action(DialogAction.customClick(
                        (r, a) -> a.showDialog(this.schedulingMenu(a))
                        , Dialogue.CLICK_CALLBACK)).build());

        LocalTime scheduledTime = faction.shieldDailyScheduledTime();
        if (scheduledTime == null) {
            ...
            dialogBody = Dialogue.body(tl.getScheduleMenuStatusBodyCurrentlySet(),
                    factionResolver,
                    Placeholder.unparsed("currenttime", LocalTime.now().format(tl.getScheduleMenuTimeFormat())),
                    Placeholder.unparsed("scheduledtime", scheduledTime.format(tl.getScheduleMenuTimeFormat()))
            );
            buttons.add(ActionButton.builder(Mini.parse(tl.getScheduleMenuStatusButtonClearSchedule(), fPlayer, factionResolver))
                    .action(DialogAction.customClick(
                            (r, a) -> {
                                faction.shieldDailyScheduledTime(null);
                                this.statusDialog(a);
                            }
                            , Dialogue.CLICK_CALLBACK)).build());
        }
        ...
        player.showDialog(Dialog.create(b -> b.empty()
                ...
                .type(DialogType.multiAction(
                        ...
                        tl.getScheduleMenuStatusColumns()
                ))));
    }
    ...
    private Dialog schedulingMenu(Audience audience) {
        ...
        List<DialogBody> dialogBody = Dialogue.body(tl.getScheduleMenuSetBody(),
                factionResolver,
                Placeholder.unparsed("currenttime", LocalTime.now().format(tl.getScheduleMenuTimeFormat()))
        );
        ...
        for (int x = 0; x < 48; x++) {
            ...
            timeButtons.add(ActionButton.builder(Mini.parse(tl.getScheduleMenuSetButtonTime(), fPlayer,
                    ...
                    .action(DialogAction.customClick(
                            (r, a) -> {
                                faction.shieldDailyScheduledTime(chosenTime);
                                this.statusDialog(a);
                            }
        ...
        return Dialog.create(b -> b.empty()
                .base(DialogBase.builder(Mini.parse(tl.getScheduleMenuSetTitle(), fPlayer))
                        .body(dialogBody)
                        .build())
                .type(DialogType.multiAction(
                        timeButtons,
                        ActionButton.builder(Mini.parse(tl.getScheduleMenuSetButtonCancel(), fPlayer, factionResolver))
                                .action(DialogAction.customClick(
                                        (r, a) -> this.statusDialog(a)
                                        , Dialogue.CLICK_CALLBACK)).build(),
