...
public class CmdTag extends FCommand {
    ...
    public void perform(CommandContext context) {
        ...
        if (!context.canAffordCommand(P.getInstance().conf().economy().getCostTag(), TL.COMMAND_TAG_TOCHANGE.toString())) {
            return;
        }
        ...
        if (!context.payForCommand(P.getInstance().conf().economy().getCostTag(), TL.COMMAND_TAG_TOCHANGE, TL.COMMAND_TAG_FORCHANGE)) {
            return;
        }
        ...
        for (FPlayer fplayer : FPlayers.getInstance().getOnlinePlayers()) {
            ...
            if (P.getInstance().conf().factions().chat().isBroadcastTagChanges()) {
