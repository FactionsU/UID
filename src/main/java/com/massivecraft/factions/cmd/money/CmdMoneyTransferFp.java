...
public class CmdMoneyTransferFp extends MoneyCommand {
    ...
    public void perform(CommandContext context) {
        ...
        if (success && P.getInstance().conf().logging().isMoneyTransactions()) {
            P.getInstance().log(ChatColor.stripColor(P.getInstance().txt.parse(TL.COMMAND_MONEYTRANSFERFP_TRANSFER.toString(), context.fPlayer.getName(), Econ.moneyString(amount), from.describeTo(null), to.describeTo(null))));
        }
    }
