...
public class CmdSetBoom implements Cmd {
    ...
    private void handle(org.incendo.cloud.context.CommandContext<Sender> context) {
        ...
        if (!context.sender().payForCommand(FactionsPlugin.getInstance().conf().economy().getCostNoBoom(), TL.COMMAND_BOOM_TOTOGGLE, TL.COMMAND_BOOM_FORTOGGLE)) {
            return;
        }

        faction.setPeacefulExplosionsEnabled(context.getOrDefault("state", !faction.getPeacefulExplosionsEnabled()));
