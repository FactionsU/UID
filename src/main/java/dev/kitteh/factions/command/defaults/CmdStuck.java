...
public class CmdStuck implements Cmd {
    ...
    private void handle(CommandContext<Sender> context) {
        ...
        // TODO handle delay 0
        final long delay = Math.max(1, FactionsPlugin.getInstance().conf().commands().stuck().getDelay());
        final int radius = FactionsPlugin.getInstance().conf().commands().stuck().getRadius();
        final int searchRadius = FactionsPlugin.getInstance().conf().commands().stuck().getSearchRadius();
        ...
        if (FactionsPlugin.getInstance().getStuckMap().containsKey(player.getUniqueId())) {
            ...
            if (!context.sender().payForCommand(FactionsPlugin.getInstance().conf().economy().getCostStuck(), TL.COMMAND_STUCK_TOSTUCK2, TL.COMMAND_STUCK_FORSTUCK2)) {
                return;
            }
