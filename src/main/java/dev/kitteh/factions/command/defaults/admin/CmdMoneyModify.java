    ...
    private void handle(CommandContext<Sender> context, boolean modify) {
        ...
        if (Econ.modifyBalance(faction, amount)) {
            ...
            if (FactionsPlugin.getInstance().conf().logging().isMoneyTransactions()) {
                FactionsPlugin.getInstance().log(ChatColor.stripColor(FactionsPlugin.getInstance().txt().parse(TL.COMMAND_MONEYMODIFY_MODIFIED.toString(), faction.describeTo(null), Econ.moneyString(amount))));
            }
        }
    }
}
