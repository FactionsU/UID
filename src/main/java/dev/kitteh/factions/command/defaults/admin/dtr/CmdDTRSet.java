...
public class CmdDTRSet implements Cmd {
    ...
    private void handle(CommandContext<Sender> context) {
        ...
        target.setDTR(Math.max(Math.min(amount, dtr.getMaxDTR(target)), FactionsPlugin.getInstance().conf().factions().landRaidControl().dtr().getMinDTR()));
        context.sender().msg(TL.COMMAND_DTR_MODIFY_DONE, target.describeTo(context.sender().fPlayerOrNull(), false), DTRControl.round(target.getDTR()));
    }
}
