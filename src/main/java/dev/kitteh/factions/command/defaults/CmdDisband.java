...
public class CmdDisband implements Cmd {
    ...
    private void doIt(FPlayer sender, boolean confirmed) {
        ...
        if (!faction.hasAccess(sender, PermissibleActions.DISBAND, sender.getLastStoodAt())) {
            sender.msg(TL.GENERIC_NOPERMISSION.format(PermissibleActions.DISBAND.getShortDescription()));
            return;
        }
        ...
        if (!FactionsPlugin.getInstance().getLandRaidControl().canDisbandFaction(faction, sender)) {
            return;
        }
